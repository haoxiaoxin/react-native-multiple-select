...
export default class MultiSelect extends Component {
  static propTypes = {
    ...
    addNewItemDisplay: PropTypes.string
  };
  ...
  _getSelectLabel = () => {
    ...
    return `${selectText} (${selectedItems.length} 项)`;
  };
  ...
  _getRowNew = (item, addNewItemDisplay) => (
    <TouchableOpacity
      disabled={item.disabled}
      onPress={() => this._addItem(item)}
      style={{ paddingLeft: 20, paddingRight: 20, paddingVertical: 5, paddingVertical: 5 }}
    >
      <View>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
          ...
          >
            添加 {item.name} {addNewItemDisplay}
          </Text>
        </View>
      </View>
    </TouchableOpacity>
  ...
  _renderItems = () => {
    ...
    if (canAddItems && !searchTermMatch && searchTerm.length && searchTerm.trim().length) {
      addItemRow = this._getRowNew({ name: searchTerm.trim() }, addNewItemDisplay);
    }
    component = (
      <View>
        {itemList}
        ...
        <View style={styles.selectorView(fixedHeight)}>
          <View style={styles.inputGroup}>
            ...
            <TextInput
              ...
              value={searchTerm}
              onFocus={() => this.setState({ selector: true })}
            />
